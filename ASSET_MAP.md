...
| PIMPA `MultiRiskFactorSimulation`/`ScenarioGenerator`/`RiskFactor`/`SimulatedData`/`CorrelationMatrix` | `src/climateCCR/simulation/` | DONE | path-major; moved 2026-06-29 (`CCR-MIG-05`). MC draw now seeded via `infra.get_legacy_rng` (`CODE_REVIEW` C4, stream-preserving, `CCR-MIG-08`); event-injection hook still to add (`DC-CCR-SIM-1`). ABCs `ScenarioGenerator`/`SimulatedData` folded into their single implementations 2026-07-11 (`CCR-MIG-09`). |
