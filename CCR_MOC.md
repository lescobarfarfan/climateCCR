...
- Reading: [[2026-08-27_fase_producer_riders]] — the fase-producer read-log: the NGFS baseline semantics behind the raw-basing ruling, the inhomogeneous-Poisson discretization behind the λ(t) rider's step-start evaluation, the $B(t,T)$ propagation behind the fase-vs-nivel gap, and the actuarial frequency-trend framing of the rider pair (`INT-34`; `notes/reading/`, `GEN-21`).
- Explanation: [[2026-08-27_fase_producer_results_explained]] — the fase producer results explained: the three fragments' path shapes (HWTP catch-up, SWUC late-abrupt, DAPS_NAM recovery), the −2.31% vs −8.43% transition gap and its three drivers, the in-simulation jump separability, the 0D-pin reporting rule, the λ(t) rider bracket and its modest −4.11%, and the stated limits (`INT-34`, `OQ-INT-12`; `notes/summary_explanations/`, `GEN-26`).
- Pipeline: [[ngfs_application_flavors]] — the two-workflow recipe for the NGFS flavors: nivel/trayectoria (shocked book roots via `pipelines/16` + `--data-root`) vs fase (the same book + `pipelines/22` fragment + `--choques-programados`, the valuation-side spread leg), the like-for-like readouts, the 0D / DAPS transition-only / CVA t=0-DF conventions and the archived vintages (`OQ-INT-12` c; `notes/pipelines/`).
- Reading: [[2026-09-08_fase_matrix_spread_leg]] — the fase-matrix + spread-leg read-log: the CLIMACRED spread-family definitions behind the excl-policy choice, the CVA bilinear decomposition behind the fase sign reversal, the $B(t,T)$ loading and the prevailing-level spread as a per-date term structure, the supervisory application conventions behind the nivel-headline ruling, and the time-averaged intensity behind the rider footnote (`INT-35`/`INT-36`; `notes/reading/`, `GEN-21`).
- Explanation: [[2026-09-08_fase_matrix_spread_leg_explained]] — the three-flavor table and how to read it (nivel −5.73/−8.18/−20.03, trayectoria −1.50/−1.25/+0.16, fase −4.57/−6.63/−7.37 %), in-simulation separability of the physical channel, the per-pillar profile and the 0D rule, what the spread schedule $\max(s_0 + \Delta s(t), 0)$ is and is not, why transition CVA rises under fase, the λ(t) rider footnote, and the limits (`INT-35`, `INT-36`; `notes/summary_explanations/`, `GEN-26`).
