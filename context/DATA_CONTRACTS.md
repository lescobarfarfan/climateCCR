...
- `DC-CCR-RISK-2` **PIMPA regression fixture + golden baseline.** Prototype portfolio under `tests/fixtures/pimpa/` (`RFs_attributes`, `calibration_data`, `market_data`, `portfolio_data`; data root via the `PIMPA_DATA_ROOT` env var). Golden at `tests/fixtures/pimpa/baselines/ee_pe_baseline.csv` — one block per `netting_agreement_id` over the B3 default grid: `default_times`, `uncollateralized_ee`, `uncollateralized_pe_0.99`, and `collateralized_ee`/`collateralized_pe_0.99` where a VM agreement exists (NaN otherwise). Reconstructed deterministically by `tests/risk_ccr/pimpa_baseline.py` (seed 233423, n_paths 10000, `today_date=2020-01-01`); guarded by `tests/risk_ccr/test_pimpa_regression.py`. PE is a **raw** (un-floored) quantile of portfolio value → can be negative for a net-liability book (→ `OQ-CCR-08`). `FIRM` (`CCR-MIG-03/05`, `GEN-04`).
