...
- `DC-CCR-SIM-2` **`processes.jumps` → `simulation` · climate jump-injection (the integrating contract, `INT-10`).** A climate-shock component superimposed on the diffusion. **Inputs (from HAZ `calibration.impact`):** arrival **intensity** `λ(t; covariates)` (homogeneous Poisson, or Cox / doubly-stochastic with covariate-driven `λ`); a **jump-mark / impact** distribution mapping each shock to a move in the target process (price-return jump for GBM, rate jump for HW1F). **Behaviour:** for each path, draw shock times from `λ`, draw marks, and add them to the diffusive increments at the matching steps — yielding a **jump-diffusion** `dX = (diffusion) + (Σ marks at Poisson/Cox times)`. **Knobs:** which target(s) a shock hits (price / rate / both); the diffusion↔jump dependence (independent vs correlated); fixed `λ` vs trajectory `λ(t)` (`INT-12`). **Output:** path arrays in the same path-major layout, consumed unchanged by `risk.*`. **▶ Build plan (the immediate next step — the engine below it is now decomposed and infra-seeded, `CCR-MIG-05/08`):** (1) fix the interface knobs (`OQ-INT-03`) provisionally to unblock — suggest a single target (HW1F **rate**), **independent** jump↔diffusion, **homogeneous Poisson** `λ` (confirm with author first); (2) implement `processes/jumps/` superimposing the compound-Poisson term on the diffusive increments inside `MultiRiskFactorSimulation`, drawing shock times + marks from the **same infra seed** as the diffusion (`infra.get_legacy_rng`/`get_rng`) so jump-on vs baseline differ only by the climate component; (3) accept `λ` + jump-mark as **injected inputs** — synthetic/stub until HAZ's `calibration.impact` supplies them (`OQ-INT-07`, `DC-XWALK-4`); (4) lock a jump regression fixture (jump-off ≡ current EE/PE baseline; jump-on = a documented deterministic shift; `GEN-04/11`). `PROPOSED` (mechanism specified; the open knobs are `OQ-INT-03/07`). `[ContTankov2004]`
