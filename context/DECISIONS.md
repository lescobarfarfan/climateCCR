...
- `CCR-MIG-07` [2026-06-29] **Inherited PIMPA ruff/black debt is deferred to a dedicated `style:` commit, not bundled into the move.** The Step-2 decomposition committed via `--no-verify`: it is a pure move (preserve git rename detection) and the pre-commit gate cannot pass on PIMPA's pre-existing style debt (≈79 non-auto-fixable ruff errors — E501/E721/E711 — already on `main` from Step 1's wholesale move). Reformatting in the move commit would also destroy rename detection (`GEN-09`: separate style from a move). New code (`config.py`, `pimpa_baseline.py`) is clean and passed the gate normally. [eng]
