...
- `CCR-MIG-05` [2026-06-28] PIMPA migrated **move-then-decompose**: Step 1 = wholesale move into `risk.ccr` + the minimal fixes (`CCR-MIG-02/04`), with the **golden EE/PE baseline locked first** as the safety net (`CCR-MIG-03`): seed 233423, n_paths 10000, valuation `today_date=2020-01-01` (latest fixture fixing; not recorded in PIMPA, fixed for reproducibility), 4 counterparties; deterministic (0.0 diff across runs). Public API name kept verbatim **`CCR_Valuation_Session`** (resolves `OQ-CCR-01`); prototype CSVs in `tests/fixtures/pimpa/` with deterministic reconstructor `tests/risk_ccr/pimpa_baseline.py` (resolves location half of `OQ-CCR-06`; `GEN-04/11`). Step 2 (split into `processes`/`simulation`/`data.market`/`calibration.financial`) proceeds under the green baseline. **Step-2 target homes:** diffusions (BM/GBM/HW1F + base) → `processes.diffusions`; orchestrator + `ScenarioGenerator`/`RiskFactor`/`CorrelationMatrix`/`SimulatedData`/`SimulatedHW1FCurve` → `simulation`; `Curve`/`Surface` → `data.market`; `MarketDataBuilder` → `calibration.financial`; shared `calendar_utils` (date/tenor math, imported by all layers) → a new top-level **`climateCCR/utils/`** (below all consumers, decided 2026-06-28); `notebook_tools` stays a CCR-local dev helper. Then YAML config (`GEN-08`/`CCR-ARCH-04`). [eng]
