...
- **EE** — expected exposure (mean positive exposure at a future grid point; the engine floors at path level, $\mathrm{mean}(\max(V,0))$). **EPE** — expected positive exposure: the time-average of the EE profile (trapezoid, year fractions — `viz.epe_summary`); the `INT-23` headline readout (the band's current state lives in `INT-23`, re-based in place). **Effective EPE** — its Basel running-max version (not built; IMM capital is EAD = α·EEPE, `[BaselCRE]`).
