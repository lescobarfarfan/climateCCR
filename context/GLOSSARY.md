...
- **Scheduled shocks (the fase flavor)** — the deterministic in-simulation scenario overlay (`INT-33`, `OQ-INT-12`): NGFS transition paths applied *at simulation dates* through the jump-overlay seam (`ScheduledShockOverlay`), zero RNG, identical across MC paths, t=0 pinned to the observed market. Completes the scenario-application triple: **nivel** = t=0 signed-peak state, **trayectoria** = maturity-dated t=0 curve reshape (`MKT-NGFS-09`), **fase** = the path arriving inside the simulation.
