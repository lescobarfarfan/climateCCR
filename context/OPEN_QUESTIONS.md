...
- `OQ-INT-03` **Jump-channel knobs + scenario taxonomies.** ⏭️ **This gates the `DC-CCR-SIM-2` build — the immediate next step (see its ▶ build plan in `DATA_CONTRACTS.md`).** Given the jump mechanism (`INT-10`, `DC-CCR-SIM-2`): (a) does a climate shock hit the **asset price (GBM), the rate (HW1F), or both**, and with what dependence to the diffusion? (b) **independent Poisson vs Cox / doubly-stochastic** `λ(t)`? (c) is there a defensible **NGFS↔SSP** mapping (`DC-XWALK-6`), or do transition (NGFS, fixed/trajectory rate) and physical (HAZ jump) channels stay deliberately distinct? These are now concrete modelling choices, not a framing gap.
