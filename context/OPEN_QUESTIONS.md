...
- `OQ-INT-09` **The loss→rate translation — the binding gap for a headline result.** The first all-estimated run (`INT-17`, price channel only) barely moves the rate-dominated fixture book: the large `INT-14` demo shifts came mostly from *placeholder rate marks*, and a loss in MDP has no estimated basis-point equivalent — `K_eff` is a price-channel object. Route candidates: an event study of CETES/TIIE (and curve-pillar) moves around major CENAPRED events, or a fiscal/sovereign-spread channel argument. Until fixed, the rate channel stays demo-placeholder and headline EE/PE results are price-channel-only. (Couples `OQ-INT-02`; the SIE plumbing of `DC-XWALK-5` helps.)
