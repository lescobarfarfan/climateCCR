...
- `OQ-INT-07` **Jump-mark estimation — the empirical climate→price impact.** `λ` (frequency) is well-served by the HAZ panels; the **jump-mark / impact distribution** (how much a given climate event moves a specific price or rate) is the harder estimation. Specify the target series, the event→return mapping, and whether the mark is calibrated from market reactions, from loss-to-price translation, or assumed from scenarios. This is the empirical core of `INT-09`. **Interface gap noted 2026-07-05:** the expected empirical shape — arrival frequency still rising (perhaps decelerating) **with severity increasing over time** (`[IPCC_AR6]` §99, to confirm) — is only half-supported by the built channel: trajectory `λ(t)` / Cox intensity already enter with no engine change (`INT-13`), but `MarkSampler.sample` is **time-blind**, so a severity *trend* needs step-aware samplers — a contained `processes.jumps` extension to spec alongside the HAZ estimates.
