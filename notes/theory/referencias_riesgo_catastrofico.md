...
- **Errores de magnitud en agro — superficies ×1000 y `SUMA ASEGURADA` ×FIX (ver recuadro abajo).** Corregidos en copia (`emision_corregida.csv`/`siniestros_corregida.csv` + `_correcciones_dq.csv`); usar SIEMPRE las copias corregidas para tasas por hectárea y series de suma asegurada del ramo agrícola.
...
> ### ⚠️ Punto fino: errores de magnitud en el ramo agrícola → correcciones ÷1000 y ÷FIX aplicadas (CAVEAT al interpretar)
> **Qué pasa.** El consolidado agrícola trae **dos errores de magnitud** de origen (vienen así en los crudos SESA de la CNSF, verificado contra `2015 Agricola Bases.xlsx`; no los introdujo la consolidación):
> 1. **Superficies ×1000** (clúster 2015; casos 2010–2024): superficie asegurada/siniestrada físicamente imposible (Michoacán-aguacate 2015 = 13.9 M ha ≈ 2.4× el territorio estatal; Sinaloa-maíz dulce 2015 = 6.1 M ha; Chiapas-maíz siniestros 2010 = 49.5 M ha) con **montos intactos**: el valor implícito cae a <100 MXN/ha y ÷1000 lo devuelve exactamente al rango del historial propio de cada celda.
> 2. **`SUMA ASEGURADA` ≈×FIX, sistémico 2022–2024** (≥12 estados, concentrado en "Seguro agrícola a la inversión (por planta)"): superficies y prima **normales**, suma inflada ~el tipo de cambio del año (consistente con una **doble conversión** MXN→"USD"→MXN en la entrega); la tasa de prima colapsa a <0.5% vs 2.7–7.5% de mediana histórica. Tras ÷FIX, la tasa implícita mediana regresa a 3.1% y el 89% de los renglones cae en banda plausible [1k, 200k] MXN/ha. La firma explota de ≤26 renglones/año (≤2021) a 224/211/196 (2022/23/24).
>
> **Regla (codificada en `limpieza_cnsf.py` §6, aplicada por `corregir_consolidados_agricola.py`).** Correcciones **en copia**: los originales `emision.csv`/`siniestros.csv` quedan "as published"; las copias `emision_corregida.csv`/`siniestros_corregida.csv` marcan cada renglón tocado (`dq_correccion`, `dq_valor_original`) y `_correcciones_dq.csv` es la auditoría renglón a renglón (915 correcciones: 25 superficies em + 97 superficies sin ÷1000; 793 sumas ÷FIX). FIX promedio del periodo (Banxico, Informe Anual — compilación 2024): 2022 = 20.1274, 2023 = 17.7587, 2024 = 18.3049.
>
> **Caveat honesto (leer antes de interpretar la serie agrícola).** (a) El ÷FIX anual es una **aproximación documentada**: el factor real por renglón depende de la fecha de emisión intra-año (FIX diario), así que los niveles 2022–2024 corregidos llevan una incertidumbre de ±5–10%. (b) **132 renglones con firma débil** (prima ≤ 0 y sin historial propio que la confirme, p. ej. arándano/agave "por planta" con `UNIDADES ASEGURADAS`=0) quedaron **sin corregir** a propósito: siguen inflados en las copias y se listan al correr el corrector — revisión manual pendiente. (c) La atribución a aseguradora es imposible desde estos archivos (no traen esa columna); la confirmación externa (fe de erratas CNSF / INAI) sigue abierta. (d) La corrección ÷1000 sí es de factor exacto y cruza validación emisión↔siniestros. Detalle y evidencia completa: impactcal-mx `scraps/cnsf_agricola_dq/` (barrido, hallazgos y figuras).
...
## Changelog
- **v0.19 (2026-07-18):** **correcciones de magnitud del ramo agrícola** (§4, recuadro nuevo): (1) superficies ×1000 (clúster 2015 descubierto vía Maíz dulce/Sinaloa; 122 renglones em+sin) corregidas ÷1000 — factor exacto, validado contra el historial de cada celda y la consistencia emisión↔siniestros; (2) `SUMA ASEGURADA` sistémicamente inflada ≈×FIX en 2022–2024 (doble conversión MXN→USD→MXN; 793 renglones) corregida ÷FIX promedio anual Banxico (20.1274/17.7587/18.3049) — tasa de prima implícita regresa a 3.1% mediana; 132 renglones de firma débil sin corregir (revisión manual). Codificado en `limpieza_cnsf.py` §6 + `corregir_consolidados_agricola.py`; salidas **en copia** (`*_corregida.csv` + `_correcciones_dq.csv` con auditoría renglón a renglón); originales intactos. Barrido completo de calidad (2,133 hallazgos en 13 chequeos, cota física del Marco Geoestadístico, bandas de valor implícito, saltos robustos) en impactcal-mx `scraps/cnsf_agricola_dq/`.
