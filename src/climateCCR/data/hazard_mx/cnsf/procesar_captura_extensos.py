...
from procesar_cenapred import _norm, a_numero, mapear_peril, separar_estados  # noqa: E402
...
def validar_captura(df: pd.DataFrame, anio: int):
    ...
    if len(malos):
        raise RuntimeError(
            f"captura {anio}: fenomeno_nivel1 inválido en filas "
    ...
    if len(sin_pagina):
        raise RuntimeError(
            f"captura {anio}: {len(sin_pagina)} filas SIN pagina_pdf — "
    ...
    if control is None:
        log.warning(
            "captura %d: SIN cifra de control registrada en CIFRAS_CONTROL; "
        ...
        log.info(
            "captura %d: total %.1f MDP vs control %.1f MDP (desv %.2f%%) OK",
            anio,
            total,
            control,
            100 * desv,
...
def procesar_capturas(dir_captura: Path = DIR_CAPTURA, dir_cons: Path = DIR_CONS):
    ...
    if not archivos:
        log.warning(
            "no hay capturas en %s (esquema en plantillas/captura_extenso_PLANTILLA.csv)",
            dir_captura,
        )
        return None, None
    eventos = []
    ...
    log.info(
        "[A] panel extensos: %d filas -> %s | [B] eventos: %d -> %s | "
