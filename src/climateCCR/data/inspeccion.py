...
Paso de cordura ANTES de calibrar sobre cualquier consolidado (CNSF, CENAPRED, …):
consolidar → **inspeccionar** → corregir (p. ej. `corregir_consolidados_agricola.py`)
→ limpiar (`limpieza_cnsf.py`). Los hallazgos requieren triaje humano; no corrige nada.

Uso (salidas no versionadas, reproducibles: results/inspeccion/<fuente>/):
    python -m climateCCR.data.inspeccion datos.csv --tiempo anio \\
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="python -m climateCCR.data.inspeccion",
    ...
    parser.add_argument("--salida", help="folder de salida (default: results/inspeccion/<archivo>)")
    parser.add_argument(
    ...
    if args.salida:
        ...
        out = project_paths().results / "inspeccion" / Path(args.archivo).stem
    comando = "python -m climateCCR.data.inspeccion " + shlex.join(
