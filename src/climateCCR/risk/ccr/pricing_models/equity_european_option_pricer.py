...
class EquityEuropeanOptionPricer(PricingModel):
    ...
    def price_single_trade(
        ...
        discount_curve = underlyings[underlyings != S and underlyings != S_implied_vol]
        T = transform_dates_to_time_differences(valuation_dates[0], trade.get_attribute("maturity"))
        ...
        S_implied_vol_values = {
            t: S_implied_vol_object.get_interpolated_surface(
                K / S_0, transform_dates_to_time_differences(valuation_dates[0], t)
            )[0]
            for t in valuation_dates
        }
        ...
        for i, valuation_date in enumerate(valuation_dates):
            if valuation_date > trade.get_attribute("maturity"):
                ...
                if trade.get_attribute("put/call") == "call":
                    trade_mtms[:, i] = (
                        ls_factor
                        * trade.get_attribute("notional")
                        * self.call_pricer(S_scenarios[:, i], K, sigma, r_scenarios[:, i], t)
                    )
                elif trade.get_attribute("put/call") == "put":
                    trade_mtms[:, i] = (
                        ls_factor
                        * trade.get_attribute("notional")
                        * self.put_pricer(S_scenarios[:, i], K, sigma, r_scenarios[:, i], t)
                    )
                else:
                    raise ValueError("Undefined put/call indicator.")
