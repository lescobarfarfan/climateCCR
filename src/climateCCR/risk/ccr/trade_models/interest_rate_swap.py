...
class InterestRateSwap(Trade):
    ...
    def load_additional_trade_attributes(self, global_parameters, risk_factors):
        ...
        self.trade_attributes["fixings_schedule"], self.trade_attributes["payments_schedule"] = (
            generate_fixings_and_payments_schedule(
                first_fixing_date,
                last_fixing_date,
                first_payment_date,
                last_payment_date,
                self.trade_attributes["payments_frequency"],
            )
        )

        trade_underlyings = [self.trade_attributes["floating_rate"]]
        if (
            risk_factors.type[risk_factors["name"] == self.trade_attributes["floating_rate"]].iloc[
                0
            ][:6]
            == "SPREAD"
        ):
            trade_underlyings.append(
                risk_factors.reference[
                    risk_factors["name"] == self.trade_attributes["floating_rate"]
                ].iloc[0]
            )

        return (
            self.trade_attributes["currency"],
            trade_underlyings,
            self.trade_attributes["fixings_schedule"],
        )
