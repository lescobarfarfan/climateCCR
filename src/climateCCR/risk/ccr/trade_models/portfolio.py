...
class Portfolio:
    ...
    def __str__(self):
        ...
        if self.trade_inventory is not None:
            text += "\n- including trade_IDs: " + str(list(self.trade_inventory.keys()))
        if self.netting_sets is not None:
            text += "\n- including netting_sets: " + str(list(self.netting_sets.keys()))
    ...
    def load(self, global_parameters):
        ...
        oe_haircuts = pd.read_csv(
            global_parameters["prototype_data_paths"]["collateral_models"]
        ...
        self.load_trade_inventory(global_parameters, master_ledger, risk_factors)
        self.load_portfolio_properties(global_parameters)
        self.load_netting_sets(master_ledger)
        self.load_VM_collateral_agreements(global_parameters, master_ledger, oe_haircuts)
    ...
    def load_trade_inventory(self, global_parameters, master_ledger, risk_factors):
        ...
        for _, row in master_ledger.iterrows():
            if row["netting_agreement_id"] == self.netting_agreement_id:
                trade_type = master_ledger[master_ledger.trade_id == row.trade_id].feed.iloc[0]
                if trade_type == "IRS":
    ...
    def load_VM_collateral_agreements(self, global_parameters, master_ledger, oe_haircuts):
        ...
        for vm_agreement in self.vm_collateral_agreements.keys():
            self.vm_collateral_agreements[vm_agreement]["contractual_terms"] = pd.read_csv(
                global_parameters["prototype_data_paths"]["counterparties"]
                + str(self.netting_agreement_id)
                + "/"
                + vm_agreement
                + "_terms.csv"
            )
    ...
    def load_portfolio_properties(self, global_parameters):
        portfolio_properties = pd.read_csv(
            global_parameters["prototype_data_paths"]["counterparties"]
            + str(self.netting_agreement_id)
            + "/counterparty_properties.csv"
        )
        self.settlement_currency = portfolio_properties[
            portfolio_properties.netting_agreement_id == self.netting_agreement_id
        ].settlement_currency.iloc[0]
