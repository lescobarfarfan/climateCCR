...
class Trade:
    ...
    def load(self, global_parameters, risk_factors):
        trade_set = pd.read_csv(
            global_parameters["prototype_data_paths"]["trades"][self.trade_asset_class]
            + global_parameters["prototype_data_files"]["trades"][self.trade_asset_class][
                self.trade_type
            ]
        )
        ...
        if self.trade_attributes is not None:
            self.trade_currency, self.trade_underlyings, self.trade_cashflows_dates = (
                self.load_additional_trade_attributes(global_parameters, risk_factors)
            )
