...
def simulate_single_risk_factor(model, simulation_dates, number_paths, number_of_risk_drivers=1):
    ...
    return pd.DataFrame(
        model.simulate(simulation_dates, random_increments), columns=simulation_dates
    )
...
def plot_rfe_paths(paths, simulation_dates, model_name):
    ...
    times = transform_dates_to_time_differences(simulation_dates[0], simulation_dates)
...
def test_rfe_mean_and_vola(paths, simulation_dates, model):
    ...
    if model.return_type == "linear":
        ...
        vola_simulation = np.apply_along_axis(np.std, 0, np.log(paths / paths[0, 0]))
    else:
        raise ValueError(f"Return type {model.return_type} is not implemented.")

    mean_exact = model.mean(times)
    vola_exact = model.volatility(times)

    fig, axarr = plt.subplots(1, 2)
    fig.suptitle("Comparison of Mean and Volatility for " + model_name, fontsize=25)
    fig.set_figheight(6)
    fig.set_figwidth(12)
...
def test_scenarios_marginal_distributions(paths, scenario_object):
    for rf in scenario_object.simulated_risk_factors:
        test_rfe_mean_and_vola(paths[rf.name], paths[rf.name + "_dates"], rf.model)
...
def test_scenarios_correlations(paths, scenario_object, correlation_matrix):
    ...
    for rf in scenario_object.simulated_risk_factors:
        if rf.model.return_type == "linear":
            ...
            raise ValueError(f"Return type {rf.model.return_type} is not implemented.")
    ...
    for rf1 in scenario_object.simulated_risk_factors:
        for rf2 in scenario_object.simulated_risk_factors:
            if rf1.name <= rf2.name:
                ...
                realised_correlations.append(pearsonr(returns[rf1.name], returns[rf2.name])[0])
                true_correlations.append(correlation_matrix.get_value(rf1.name, rf2.name))

    result = pd.DataFrame(
        index=index,
        data={
            "realised_correlations": realised_correlations,
