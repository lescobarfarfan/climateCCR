...
class SimulatedHW1FCurve:
    ...
    def get_value(self, **kwargs):
        if kwargs["initial_date"] is not None:
            t = transform_dates_to_time_differences(kwargs["initial_date"], kwargs["t_date"])
            T = transform_dates_to_time_differences(kwargs["initial_date"], kwargs["T_date"])
        else:
            t = kwargs["t_date"]
            T = kwargs["T_date"]
    ...
    def HW1F_A_tT(self, model_parameters, t_date, T_date, B=None, initial_date=None):
        ...
        P_t = model_parameters["rate_curve"].get_interpolated_discount_factor(t)
        P_T = model_parameters["rate_curve"].get_interpolated_discount_factor(T)
        F_t = model_parameters["rate_curve"].get_interpolated_instantaneous_forward_rate()(t)
        if B is None:
        ...
        return np.reshape(
            ...
            * np.exp(
                B * F_t
                - (model_parameters["volatility"] ** 2)
                / (4 * model_parameters["alpha"])
                * (1 - np.exp(-2 * model_parameters["alpha"] * t))
                * B**2
            ),
            (1, -1),
        )
