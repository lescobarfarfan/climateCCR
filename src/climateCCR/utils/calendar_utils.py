...
def generate_simulation_dates_schedule(
    starting_date, final_date, valuation_frequency, global_parameters
):
    starting_date = datetime.strptime(starting_date, global_parameters["date_format"])
    final_date = datetime.strptime(final_date, global_parameters["date_format"])
...
def generate_payments_schedule(first_payment_date, last_payment_date, payments_frequency):
    ...
    return generate_payment_schedule_from_time_steps(
        first_payment_date, last_payment_date, time_step
    )


def generate_fixings_and_payments_schedule(
    first_fixing_date, last_fixing_date, first_payment_date, last_payment_date, payments_frequency
):
    if (
        (first_fixing_date >= last_fixing_date)
        or (first_payment_date >= last_payment_date)
        or (first_fixing_date > first_payment_date)
        or (last_fixing_date > last_payment_date)
    ):
        raise ValueError("Provided fixing and payments dates are not consistent")

    time_step = time_step_from_frequency(payments_frequency)
    return generate_payment_schedule_from_time_steps(
        first_fixing_date, last_fixing_date, time_step
    ), generate_payment_schedule_from_time_steps(first_payment_date, last_payment_date, time_step)
