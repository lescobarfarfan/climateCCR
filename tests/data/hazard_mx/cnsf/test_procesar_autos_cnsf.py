...
DATA = {
    2008: {
        ...
        "Emision": pd.DataFrame({
            "Entidad": ["01"], "Marca": ["0124"], "Tipo de Vehiculo": ["01"],
            "Cobertura": ["06"], "Deducible": ["0"],
            "Numero de Vehiculos": ["10"], "Prima Emitida": ["100.5"],
        }),
    ...
    2024: {
        ...
        "Emision": pd.DataFrame({
            "Entidad": ["17", "01"], "Marca": ["0124", "ND"],
            "Tipo de Vehiculo": ["01", "01"], "Forma de Venta": ["05", "05"],
            "Cobertura": ["06", "06"], "Subtipo de Seguro": ["0", "0"],
            "Deducible": ["0", "1"], "Numero de Vehiculos": ["5", "3"],
...
def test_descubrir_subcarpetas():
    ...
    config = json.loads(Path("catalogos_autos_cnsf.json").read_text(encoding="utf-8"))
...
def test_flotilla_normalizado():
    ...
    assert out.loc[0, "Tipo de vehiculo_desc"] == "Automovil"       # 'Tipo de Vehiculo' (config) ~ 'Tipo de vehiculo'
    assert out.loc[0, "Tipo de perdida_desc"] == "Robo total"
    assert out.loc[0, "Causa del Siniestro_desc"] == "Choque"       # 'Causa del siniestro' (config) ~ 'Causa del Siniestro'
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp) / "automoviles"; root.mkdir()
        ...
        for c in ("Entidad", "Entidad_desc", "Marca", "Marca_desc", "Tipo_desc",
                  "Forma de Venta", "Forma de Venta_desc", "Subtipo de Seguro",
        ...
        assert fila0["Marca"] == "0124" and fila0["Marca_desc"] == "NISSAN" and fila0["Tipo_desc"] == "SEDAN"
        assert fila0["Subtipo de Seguro"] == "0" and fila0["Subtipo de Seguro_desc"] == "Tradicional", \
        ...
        assert fila_nd["Marca"] == "ND" and fila_nd["Marca_desc"] == "No disponible" \
            and fila_nd["Tipo_desc"] == "No disponible", "ND debe etiquetarse, no perderse"

        # deriva 2008: columnas canónicas ausentes -> vacías; sus _desc vacíos
        f2008 = df[df["anio"] == "2008"].reset_index(drop=True).iloc[0]
        assert f2008["Forma de Venta"] == "" and f2008["Forma de Venta_desc"] == "", \
